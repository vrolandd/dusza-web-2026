<script lang="ts">
	...
	// Stabil, ABC sorrendű lista
	$: filteredImages = cardType ? [...images[cardType]].sort() : [];

	// Csak akkor állítjuk az első képet, ha nincs kiválasztva, vagy nem található az új listában
	$: if (cardType && (!cardImage || !filteredImages.includes(cardImage))) {
		cardImage = filteredImages[0];
	}
